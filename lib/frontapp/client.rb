    ...
    def format_query(params)
      ...
      if q && q.is_a?(Hash)
        res << q.map do |k, v|
          case v
          when Symbol, String
            "q[#{k}]=#{CGI.escapeURIComponent(v)}"
          when Array then
            v.map { |c| "q[#{k}][]=#{CGI.escapeURIComponent(c.to_s)}" }.join("&")
          else
            "q[#{k}]=#{CGI.escapeURIComponent(v.to_s)}"
          end
        end
      end
      res << params.map {|k,v| "#{k}=#{CGI.escapeURIComponent(v.to_s)}"}
      res.join("&")
    end
